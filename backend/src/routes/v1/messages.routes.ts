...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    const list = await Conversation.find({ participants: userId })
      ...
      .lean(); // returns plain JS objects (not mongoose documents) for performance
    ...
    const unreadMap = unreadAgg.reduce((acc: any, r: any) => {
      acc[r._id.toString()] = r.count;
      return acc;
    }, {});
    const withUnread = list.map((c: any) => ({
      ...c,
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    if (!allowed) {
      return res
        .status(403)
        .json({
          message: "Messaging allowed only after an accepted swap request.",
        });
    }
