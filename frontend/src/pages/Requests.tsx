...
const Requests: React.FC = () => {
  ...
  useEffect(() => {
    ...
    getUserSessionsPaged(params)
      .then(setResult)
      .catch(() =>
        addToast({ type: "error", message: "Failed to load requests" })
      )
  ...
  const outgoing = (result?.data || []).filter(
    (s) => s.fromUser._id === user?._id
  );
  const incoming = (result?.data || []).filter(
    (s) => s.toUser._id === user?._id
  );
            ...
            {incoming.length === 0 ? (
              ...
              <ul className="space-y-3 text-sm">
                {incoming.map((s) => (
                  <li
                    key={s._id}
                    className="rounded-lg border border-blue-200 shadow-sm p-3 bg-white"
                  >
                    From <span className="font-medium">{s.fromUser.name}</span>{" "}
                    ...
                    {s.message && (
                      <div className="mt-1 text-gray-700">"{s.message}"</div>
                    )}
              ...
              <ul className="space-y-3 text-sm">
                {outgoing.map((s) => (
                  <li
                    ...
                    className="rounded-lg border border-blue-200 shadow-sm p-3 bg-white"
                  >
                    To <span className="font-medium">{s.toUser.name}</span> -{" "}
                    {s.fromUserSkill} - {s.toUserSkill}
                    <div className="text-xs text-gray-500">
                      Status: {s.status}
                      {s.availability ? " | " + s.availability : ""}
                ...
                {pageNumbers.map((n) => (
                  <button
                    ...
                    className={`rounded border px-3 py-1 text-sm ${
                      n === (params.page ?? 1) ? "bg-black text-white" : ""
                    }`}
